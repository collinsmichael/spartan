...
bool OnLoad(CWindow *win, CEvent *evt) {
    ...
    if (parent) {
        parent->Title = "Speaker";
    }
    win->BackGround[0] = 0x00E0E0E0;
    win->ForeGround[0] = 0x001E1E1E;

    if (!Bitmap->LoadSprite(&icon[0], "Apps/Settings/Mute.bmp")) {
        return false;
    }
    if (!Bitmap->LoadSprite(&icon[1], "Apps/Settings/Volume.bmp")) {
        return false;
    }
    if (!Bitmap->LoadSprite(&icon[2], "Apps/Settings/Speaker.bmp")) {
        return false;
    }
...
bool OnDraw(CWindow *win, CEvent *evt) {
    ...
    Window->TextOut(win, tx, ty, "%s", "volume");
...
bool OnAction(CWindow *win, CEvent *evt) {
    ...
    if (progress) {
        ...
        if (image) {
            if (progress->CurValue == progress->MinValue) {
                image->Bitmap = &icon[0];
            } else if (progress->CurValue == progress->MaxValue) {
                image->Bitmap = &icon[2];
            } else {
                image->Bitmap = &icon[1];
            }
