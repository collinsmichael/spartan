...
bool OnLoad(CWindow *win, CEvent *evt) {
    ...
    if (!Bitmap->LoadSprite(&icon, "Apps/Settings/Mouse.bmp")) {
        return false;
    }
    ...
    if (progress) {
        ...
        progress->MinValue  = 16;
        progress->CurValue  = Mouse->GetSpeed();
...
bool OnDraw(CWindow *win, CEvent *evt) {
    ...
    int ty = progress->PosY - font->FontH;
    Window->TextOut(win, tx, ty, "%s", "mouse speed");

    int speed = Mouse->GetSpeed();
    progress->CurValue = speed;
...
bool OnAction(CWindow *win, CEvent *evt) {
    ...
    if (progress) {
        ...
        case 0x1001: // less
            ...
            Mouse->SetSpeed(progress->CurValue);
            progress->CurValue = Mouse->GetSpeed();
            break;
        case 0x1002: // more
