...
/* ************************************************************************** */
_declspec(dllexport)
int renode(int inode, uint32_t size) {
    // try to grow the current disk block
    uint32_t delta = size - inodes[inode].capacity;
    uint32_t start = inodes[inode].base + inodes[inode].capacity;
