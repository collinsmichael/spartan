...
RelocationDone:
       ...
       ;add     ax, [ds:16h]            ; cs (you're wrong BootProg)
       ;push    ax                      ; correct is as follows
        push    word [ds:16h]           ; cs
        push    word [ds:14h]           ; ip
Run:
...
ReadCluster:
        ...
        add     ax, word [ss:bp+1*2]
        adc     dx, word [ss:bp+2*2]
                ; dx:ax = LBA
        ...
        push    ds
        mov     ds, word [ss:bp+3*2]         ; ds = FAT segment
        mov     si, word [ds:si]             ; si = next cluster
        pop     ds
...
ReadSectorDone:
        ...
        add     bx, word [bpbBytesPerSector] ; adjust offset for next sector
        add     ax, 1
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
ProgramName     db      "RAMDISK IMG"   ; name and extension must be padded
